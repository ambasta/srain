...
static void irc_event_error(SircSession *sirc, const char *event,
                            const char *origin, const char **params, int count);
static void irc_event_welcome(SircSession *sirc, int event, const char *origin,
                              const char *params[], int count);
static void irc_event_numeric(SircSession *sirc, int event, const char *origin,
                              const char *params[], int count);
...
static void irc_event_connect(SircSession *sirc, const char *event) {
  ...
  srn_chat_add_misc_message_fmt(srv->chat, _("Connected to %1$s(%2$s:%3$d)"),
                                srv->name, srv->addr->host, srv->addr->port);
  list = srv->chat_list;
  while (list) {
    chat = list->data;
    srn_chat_add_misc_message_fmt(chat, _("Connected to %1$s(%2$s:%3$d)"),
                                  srv->name, srv->addr->host, srv->addr->port);
...
static void irc_event_connect_fail(SircSession *sirc, const char *event,
  ...
  while (list) {
    ...
    chat = list->data;
    srn_chat_add_misc_message_fmt(
        chat, _("Failed to connect to %1$s(%2$s:%3$d): %4$s"), srv->name,
        srv->addr->host, srv->addr->port, msg);
    if (srv->state == SRN_SERVER_STATE_RECONNECTING) {
      srn_chat_add_misc_message_fmt(
          chat, _("Trying reconnect to %1$s(%2$s:%3$d) after %4$.1lfs..."),
          srv->name, srv->addr->host, srv->addr->port,
  ...
  srn_chat_add_error_message_fmt(
      srv->chat, _("Failed to connect to %1$s(%2$s:%3$d): %4$s"), srv->name,
      srv->addr->host, srv->addr->port, msg);
  ...
  if (!srv->cfg->irc->tls &&
    ...
    srn_chat_add_error_message_fmt(
        srv->chat,
        _("It seems that you connect to a TLS port(%1$d) without enable TLS "
          "connection, try to enable it and reconnect"),
        srv->addr->port);
  }
  if (srv->state == SRN_SERVER_STATE_RECONNECTING) {
    srn_chat_add_misc_message_fmt(
        srv->chat, _("Trying reconnect to %1$s(%2$s:%3$d) after %4$.1lfs..."),
        srv->name, srv->addr->host, srv->addr->port,
        (srv->reconn_interval * 1.0) / 1000);
  }
}
...
static void irc_event_disconnect(SircSession *sirc, const char *event,
  ...
  while (list) {
    ...
    srn_chat_add_misc_message_fmt(
        chat, _("Disconnected from %1$s(%2$s:%3$d): %4$s"), srv->name,
        srv->addr->host, srv->addr->port, msg);
    if (srv->state == SRN_SERVER_STATE_RECONNECTING) {
      srn_chat_add_misc_message_fmt(
          chat, _("Trying reconnect to %1$s(%2$s:%3$d) after %4$.1lfs..."),
  ...
  srn_chat_add_error_message_fmt(
      srv->chat, _("Disconnected from %1$s(%2$s:%3$d): %4$s"), srv->name,
      srv->addr->host, srv->addr->port, msg);
  if (srv->state == SRN_SERVER_STATE_RECONNECTING) {
    srn_chat_add_misc_message_fmt(
        srv->chat, _("Trying reconnect to %1$s(%2$s:%3$d) after %4$.1lfs..."),
...
static void irc_event_welcome(SircSession *sirc, int event, const char *origin,
  ...
  srv->ping_timer =
      g_timeout_add(SRN_SERVER_PING_INTERVAL, irc_period_ping, srv);
  ...
  nick_match = !strcmp(g_utf8_casefold(srv->cfg->user->nick, -1),
                       g_utf8_casefold(nick, -1));
  ...
  case SRN_LOGIN_METHOD_MSG_NICKSERV: {
    ...
    msg = g_strdup_printf("IDENTIFY %s", srv->cfg->user->login->password);
  ...
  if (try_login) {
    if (nick_match) {
      srn_chat_add_misc_message_fmt(
          srv->chat, _("Logging in with %1$s..."),
          srn_login_method_to_string(srv->cfg->user->login->method));
    } else {
      srn_chat_add_error_message(srv->chat,
                                 _("The assigned nickname does not match the "
                                   "requested nickname, login skipped"));
    }
  }
...
static void irc_event_mode(SircSession *sirc, const char *event,
  ...
  do {
    ...
    if (mode[0] == '-') {
      ...
      ERR_FR("Unrecognized mode: %s. chan: %s, mode_args: %s", mode, chan,
             mode_args);
    }
...
static void irc_event_topic(SircSession *sirc, const char *event,
  ...
  if (strlen(topic) == 0) {
    srn_chat_add_misc_message_with_user_fmt(
        chat, chat_user, _("%1$s cleared topic"), origin, topic);
  } else {
    srn_chat_add_misc_message_with_user_fmt(
        chat, chat_user, _("%1$s changed topic to:\n\t%2$s"), origin, topic);
  }
}
...
static void irc_event_kick(SircSession *sirc, const char *event,
  ...
  if (kicked_chat_user->srv_user->is_me) {
    if (reason) {
      snprintf(buf, sizeof(buf), _("You are kicked by %1$s: %2$s"), origin,
               reason);
    } else {
      snprintf(buf, sizeof(buf), _("You are kicked by %1$s"), origin);
    }
    srn_chat_set_is_joined(chat, FALSE);
  } else {
    if (reason) {
      snprintf(buf, sizeof(buf), _("%1$s are kicked by %2$s: %3$s"), kicked,
               origin, reason);
    } else {
      snprintf(buf, sizeof(buf), _("%1$s are kicked by %2$s"), kicked, origin);
    }
  }
...
static void irc_event_invite(SircSession *sirc, const char *event,
  ...
  srn_chat_add_misc_message_with_user_fmt(
      chat, chat_user, _("%1$s invites you into %2$s"), origin, chan);
}
...
static void irc_event_ctcp_req(SircSession *sirc, const char *event,
  ...
  if (strcmp(event, "ACTION") == 0) {
    ...
    srn_chat_add_error_message_with_user_fmt(
        chat, chat_user, _("Received unsupported CTCP %1$s request form %2$s"),
        event, origin);
  } else {
    srn_chat_add_misc_message_with_user_fmt(
        chat, chat_user, _("Received CTCP %1$s request form %2$s"), event,
        origin);
  }
}
...
static void irc_event_ctcp_rsp(SircSession *sirc, const char *event,
  ...
  if (strcmp(event, "ACTION") == 0) {
  ...
  } else if (strcmp(event, "CLIENTINFO") == 0 || strcmp(event, "FINGER") == 0 ||
             strcmp(event, "SOURCE") == 0 || strcmp(event, "TIME") == 0 ||
             strcmp(event, "VERSION") == 0 || strcmp(event, "USERINFO") == 0) {
    srn_chat_add_misc_message_with_user_fmt(
        chat, chat_user, _("Received CTCP %1$s response from %2$s: %3$s"),
        event, origin, msg);
  } else if (strcmp(event, "DCC") == 0) {
    // TODO
    srn_chat_add_error_message_with_user_fmt(
        chat, chat_user, _("Received unsupported CTCP %1$s response form %2$s"),
        event, origin);
  } else if (strcmp(event, "PING") == 0) {
    unsigned long time;
    ...
    if (time != 0 && nowtime >= time) {
      ...
      srn_chat_add_misc_message_with_user_fmt(
          chat, chat_user, _("Latency between %1$s: %2$lums"), origin,
          nowtime - time);
    } else {
      srn_chat_add_misc_message_with_user_fmt(
          chat, chat_user, _("Received CTCP %1$s response from %2$s: %3$s"),
...
static void irc_event_cap(SircSession *sirc, const char *event,
  ...
  if (g_ascii_strcasecmp(cap_event, "LS") == 0) {
    ...
    srn_chat_add_misc_message_fmt(srv->chat, _("Server capabilities: %1$s"),
                                  rawcaps);
    if (buf->len > 0) {
      srn_chat_add_misc_message_fmt(
          srv->chat, _("Requesting capabilities: %1$s"), buf->str);
    ...
    srn_chat_add_misc_message_fmt(
        srv->chat, _("Server has new capabilities: %1$s"), rawcaps);
    if (buf->len > 0) {
      srn_chat_add_misc_message_fmt(
          srv->chat, _("Requesting new capabilities: %1$s"), buf->str);
      sirc_cmd_cap_req(sirc, buf->str);
    } else {
      srn_chat_add_misc_message_fmt(srv->chat,
    ...
    if (strlen(rawcaps) > 0) {
      srn_chat_add_misc_message_fmt(
          srv->chat, _("Acknowledged capabilities: %1$s"), rawcaps);
    } else {
      srn_chat_add_misc_message_fmt(srv->chat, _("No acknowledged capability"));
    }
    ...
    srn_chat_add_misc_message_fmt(
        srv->chat, _("Server has deleted capabilities: %1$s"), rawcaps);
  ...
  if (!srv->negotiated && cap_end) {
    ...
    if (method == SRN_LOGIN_METHOD_SASL_PLAIN ||
        method == SRN_LOGIN_METHOD_SASL_ECDSA_NIST256P_CHALLENGE) {
      if (srv->cap->client_enabled.sasl) {
        ...
        srn_chat_add_error_message_fmt(srv->chat,
                                       _("SASL authentication is not supported "
                                         "on this server, login skipped"));
...
static void irc_event_authenticate(SircSession *sirc, const char *event,
  ...
  case SRN_LOGIN_METHOD_SASL_PLAIN: {
    ...
    str = g_string_append_unichar(str,
                                  g_utf8_get_char("\0")); // Unicode null char
    str = g_string_append(str, srv->cfg->user->nick);
    str = g_string_append_unichar(str,
                                  g_utf8_get_char("\0")); // Unicode null char
    ...
    srn_chat_add_misc_message_fmt(srv->chat,
                                  _("Logging in with %1$s as %2$s..."), method,
                                  srv->cfg->user->nick);
  ...
  case SRN_LOGIN_METHOD_SASL_ECDSA_NIST256P_CHALLENGE: {
    ...
    // This is the second phase to authenticate, we need to sign the message
    // from server and reply it
    ...
    if (keypair == NULL) {
      ERR_FR("File %s not found, or it is not a valid ecdsa certificate file.",
             cert);
      return;
    }
    libecdsaauth_sign_base64(keypair, (unsigned char *)challenge,
                             strlen(challenge), &output, &outlen);
...
static void irc_event_numeric(SircSession *sirc, int event, const char *origin,
  ...
  case SIRC_RFC_ERR_NICKNAMEINUSE: {
    ...
    if (!srv->registered) {
      ...
      srn_chat_add_misc_message_fmt(
          srv->chat, _("Trying nickname: \"%1$s\"..."), new_nick);
  ...
  case SIRC_RFC_RPL_NAMREPLY: {
    ...
    for (nickptr = strtok(dup_names, " "); nickptr;
         nickptr = strtok(NULL, " ")) {
      switch (nickptr[0]) {
      case '~':
        nickptr++;
  ...
  case SIRC_RFC_RPL_NOTOPIC: {
    ...
    srn_chat_add_misc_message_fmt(chat, _("No topic is set"), chan);
    break;
  }
  case SIRC_RFC_RPL_TOPIC: {
    ...
    srn_chat_set_topic(chat, chat_user, topic);
    srn_chat_add_misc_message_with_user_fmt(
        chat, chat_user, _("The topic of this channel is:\n\t%1$s"), topic);
    break;
  }
  ...
  case SIRC_RFC_RPL_WHOISUSER: {
    ...
    srn_chat_add_misc_message_fmt(srv->cur_chat, _("%1$s <%2$s@%3$s> %4$s"),
  ...
  case SIRC_RFC_RPL_WHOISCHANNELS: {
    ...
    // TODO: dont show WHOIS message in message list
    srn_chat_add_misc_message_fmt(srv->cur_chat, _("%1$s is a member of %2$s"),
                                  params[1], msg);
    break;
  }
  ...
  case SIRC_RFC_RPL_WHOWAS_TIME: {
    ...
    // TODO: dont show WHOIS message in message list
    srn_chat_add_misc_message_fmt(srv->cur_chat, _("%1$s %2$s %3$s"), params[1],
                                  msg, params[2]);
    break;
  }
  ...
  case SIRC_RFC_RPL_WHOISSECURE: {
    ...
    // TODO: dont show WHOIS message in message list
    srn_chat_add_misc_message_fmt(srv->cur_chat, _("%1$s %2$s"), params[1],
                                  msg);
    break;
  }
  ...
  case SIRC_RFC_RPL_AWAY: {
    ...
    srn_chat_add_error_message_with_user_fmt(
        chat, chat_user, _("%1$s is away: %2$s"), nick, msg);
    break;
  }
  ...
  case SIRC_RFC_RPL_CHANNEL_URL: {
    ...
    srn_chat_add_misc_message_with_user_fmt(
        chat, chat_user, _("URL of %1$s is: %2$s"), chan, msg);
    break;
  }
  default: {
    // Error message
  ...
  ERRMSG:
     ...
     * is ["<nick>", ... "<reason>"] */
    {
      ...
      srn_chat_add_error_message_fmt(srv->cur_chat, _("ERROR[%1$3d] %2$s"),
                                     event, buf->str);
...
static gboolean irc_period_ping(gpointer user_data) {
  ...
  DBG_FR("Server %s, %lu ms since last pong, time out: %d ms", srv->name,
         time - srv->last_pong, SRN_SERVER_PING_TIMEOUT);
