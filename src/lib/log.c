...
void srn_logger_free(SrnLogger *logger) { g_free(logger); }

void srn_logger_set_default(SrnLogger *logger) { srn_logger = logger; }

SrnLogger *srn_logger_get_default(void) { return srn_logger; }
...
void srn_logger_log(SrnLogger *logger, SrnLogLevel lv, bool print_prompt,
  ...
  if (print_prompt) {
    ...
    g_string_printf(output, prompts[logger->cfg->prompt_color][lv],
                    prompt->str);
...
SrnRet srn_logger_config_check(SrnLoggerConfig *cfg) { return SRN_OK; }
