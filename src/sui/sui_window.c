...
static void sui_window_init(SuiWindow *self) {
  ...
  g_object_bind_property(self->title_paned, "position", self->main_paned,
                         "position", G_BINDING_BIDIRECTIONAL);
  g_object_bind_property(self->header_paned, "position", self->main_paned,
                         "position", G_BINDING_BIDIRECTIONAL);
  ...
  gtk_box_pack_start(self->side_box, GTK_WIDGET(self->side_bar), TRUE, TRUE, 0);
  sui_side_bar_set_stack(self->side_bar, self->buffer_stack);
  gtk_widget_show(GTK_WIDGET(self->side_bar));

  // Setup menu button
  gtk_menu_button_set_popover(self->start_menu_button,
  ...
  g_signal_connect(self, "notify::is-active", G_CALLBACK(on_notify_is_active),
                   NULL);
  ...
  closure_j =
      g_cclosure_new(G_CALLBACK(CTRL_J_K_on_press), self->side_bar, NULL);
  closure_k =
      g_cclosure_new(G_CALLBACK(CTRL_J_K_on_press), self->side_bar, NULL);

  gtk_accel_group_connect(accel, GDK_KEY_j, GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE,
                          closure_j);
  gtk_accel_group_connect(accel, GDK_KEY_k, GDK_CONTROL_MASK, GTK_ACCEL_VISIBLE,
                          closure_k);
...
static void sui_window_constructed(GObject *object) {
  ...
  if (!self->cfg->csd) {
    ...
    gtk_container_child_set(GTK_CONTAINER(self->side_header_box),
                            GTK_WIDGET(self->side_right_header_box), "expand",
                            FALSE, NULL);
    ...
    gtk_container_child_set(GTK_CONTAINER(self->buffer_header_box),
                            GTK_WIDGET(self->buffer_menu_button), "expand",
                            FALSE, NULL);
...
static void sui_window_class_init(SuiWindowClass *class) {
  ...
  obj_properties[PROP_EVENTS] =
      g_param_spec_pointer("events", "Events", "Event callbacks of window.",
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  obj_properties[PROP_CONFIG] =
      g_param_spec_pointer("config", "Config", "Configuration of window.",
...
SuiWindow *sui_window_new(SuiApplication *app, SuiWindowEvents *events,
  ...
  self = g_object_new(SUI_TYPE_WINDOW, "application", app, "events", events,
                      "config", cfg, NULL);
...
void sui_window_add_buffer(SuiWindow *self, SuiBuffer *buf) {
  ...
  if (get_buffer_count(self) == 0) {
    gtk_stack_set_visible_child_name(self->window_stack,
                                     WINDOW_STACK_PAGE_MAIN);
  }

  gstr = g_string_new("");
  g_string_printf(gstr, "%s %s", sui_buffer_get_remark(buf),
...
SuiBuffer *sui_window_get_buffer(SuiWindow *self, const char *name,
  ...
  buf = SUI_BUFFER(
      gtk_stack_get_child_by_name(self->buffer_stack, fullname->str));
  g_string_free(fullname, TRUE);

  return buf;
}

SuiSideBar *sui_window_get_side_bar(SuiWindow *self) { return self->side_bar; }
...
SuiWindowEvents *sui_window_get_events(SuiWindow *self) { return self->events; }

void sui_window_set_config(SuiWindow *self, SuiWindowConfig *cfg) {
  self->cfg = cfg;
}

SuiWindowConfig *sui_window_get_config(SuiWindow *self) { return self->cfg; }
...
static void update_title(SuiWindow *self) {
  ...
  if (g_strcmp0(page, WINDOW_STACK_PAGE_WELCOME) == 0) {
    ...
    title = g_strdup_printf(
        "%s @ %s - %s", gtk_label_get_text(self->buffer_title_label),
        gtk_label_get_text(self->buffer_subtitle_label), PACKAGE_NAME);
...
static void send_message(SuiWindow *self) {
  ...
  self->send_timer =
      g_timeout_add(SEND_MESSAGE_INTERVAL, send_message_timeout, self);

  gtk_image_set_from_icon_name(
      GTK_IMAGE(gtk_button_get_image(self->send_button)), "document-revert",
      GTK_ICON_SIZE_BUTTON);
}
...
static void send_message_cancel(SuiWindow *self) {
  ...
  gtk_image_set_from_icon_name(
      GTK_IMAGE(gtk_button_get_image(self->send_button)), "document-send",
      GTK_ICON_SIZE_BUTTON);
}
