...
static void listbox_on_row_selected(GtkListBox *box, GtkListBoxRow *row,
                                    gpointer user_data) {
  SuiSideBar *sidebar;
  SuiSideBarItem *item;
  GtkWidget *child;
...
static gboolean list_box_on_popup(GtkWidget *widget, GdkEventButton *event,
                                  gpointer user_data) {
  GtkListBox *list_box;
  GtkListBoxRow *row;
  SuiBuffer *child;
  ...
  if (event->button == 3) {
    ...
    gtk_menu_popup_at_pointer(sui_buffer_get_menu(child),
...
static void sui_side_bar_init(SuiSideBar *self) {
  GtkWidget *sw;
  ...
  gtk_list_box_set_sort_func(GTK_LIST_BOX(self->list), list_sort_func, NULL,
                             NULL);
...
static void add_child(GtkWidget *child, SuiSideBar *sidebar) {
  const char *icon;
  GtkListBoxRow *row;
  SuiBuffer *buf;
  SuiSideBarItem *item;
  ...
  item = sui_side_bar_item_new(sui_buffer_get_name(buf),
                               sui_buffer_get_remark(buf), icon);
  ...
  row = sui_common_add_gtk_list_box_unfocusable_row(sidebar->list,
                                                    GTK_WIDGET(item));
...
static void remove_child(GtkWidget *widget, SuiSideBar *sidebar) {
  GtkWidget *row;
...
static void populate_sidebar(SuiSideBar *sidebar) {
...
static void clear_sidebar(SuiSideBar *sidebar) {
...
static void disconnect_stack_signals(SuiSideBar *sidebar) {
...
static void connect_stack_signals(SuiSideBar *sidebar) {
...
static void sui_side_bar_dispose(GObject *object) {
...
static void sui_side_bar_finalize(GObject *object) {
  SuiSideBar *sidebar = SUI_SIDE_BAR(object);
...
static void sui_side_bar_class_init(SuiSideBarClass *klass) {
...
SuiSideBar *sui_side_bar_new() { return g_object_new(SUI_TYPE_SIDE_BAR, NULL); }

void sui_side_bar_set_stack(SuiSideBar *sidebar, GtkStack *stack) {
...
GtkStack *sui_side_bar_get_stack(SuiSideBar *sidebar) {
  g_return_val_if_fail(GTK_IS_STACK_SIDEBAR(sidebar), NULL);

  return GTK_STACK(sidebar->stack);
}

SuiSideBarItem *sui_side_bar_get_item(SuiSideBar *sidebar, SuiBuffer *buf) {
  GtkListBoxRow *row;
...
void sui_side_bar_prev(SuiSideBar *sidebar) {
  GList *rows;
  ...
  while (rows) {
    if (rows->data == cur_row) {
      if (rows->prev) {
        ...
        tail = rows->data;
        gtk_list_box_select_row(sidebar->list, GTK_LIST_BOX_ROW(tail));
        return;
      }
    }
    rows = rows->next;
  ...
  while (rows) {
    if (rows->data == cur_row) {
      if (rows->next) {
        ...
        gtk_list_box_select_row(sidebar->list, GTK_LIST_BOX_ROW(head));
      }
    }
    rows = rows->next;
  }
