...
// user_data); static void tray_icon_on_popup_menu(GtkStatusIcon *status_icon,
// guint button,
...
static void sui_application_init(SuiApplication *self) {
  ...
  g_action_map_add_action_entries(G_ACTION_MAP(self), action_entries, -1, self);
...
static void sui_application_class_init(SuiApplicationClass *class) {
  ...
  obj_properties[PROP_CTX] =
      g_param_spec_pointer("context", "Context", "Context of application.",
                           G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  obj_properties[PROP_EVENTS] = g_param_spec_pointer(
      "events", "Events", "Event callbacks of application.",
      G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  obj_properties[PROP_CONFIG] =
      g_param_spec_pointer("config", "Config", "Configuration of application.",
...
SuiApplication *sui_application_new(const char *id, void *ctx,
  ...
  if (app_instance == NULL) {
    app_instance =
        g_object_new(SUI_TYPE_APPLICATION, "application-id", id, "flags",
                     G_APPLICATION_HANDLES_COMMAND_LINE, "context", ctx,
                     "events", events, "config", cfg, NULL);
  }
...
// void sui_application_highlight_tray_icon(SuiApplication *self, bool
// highlight) {
//   gtk_status_icon_set_from_icon_name(self->tray_icon,
//                                      highlight ? "srain-red" : PACKAGE);
// }

SuiApplication *sui_application_get_instance() { return app_instance; }
...
void *sui_application_get_ctx(SuiApplication *self) { return self->ctx; }

SuiApplicationEvents *sui_application_get_events(SuiApplication *self) {
  return self->events;
}

void sui_application_set_config(SuiApplication *self,
                                SuiApplicationConfig *cfg) {
  GList *wins;
...
static void show_about_dialog(SuiApplication *self) {
  ...
  const gchar *version =
      g_strdup_printf(_("%1$s%2$s\nRunning against GTK+ %3$d.%4$d.%5$d"),
                      PACKAGE_VERSION, PACKAGE_BUILD, gtk_get_major_version(),
                      gtk_get_minor_version(), gtk_get_micro_version());
  const char *translators = "Artem Polishchuk (ru)\n"
                            "Shengyu Zhang (zh_CN)\n"
                            "Jianqiu Zhang (zh_CN)";

  gtk_show_about_dialog(
      window, "program-name", PACKAGE_NAME, "version", version, "copyright",
      "(C) " PACKAGE_COPYRIGHT_DATES " " PACKAGE_AUTHOR, "license-type",
      GTK_LICENSE_GPL_3_0, "website", PACKAGE_WEBSITE, "comments", PACKAGE_DESC,
      "authors", authors, "documenters", documentors, "logo-icon-name", PACKAGE,
      "title", _("About Srain"), "translator-credits", translators, NULL);
}
...
static void on_startup(SuiApplication *self) {
  ...
  //  self->tray_icon = GTK_STATUS_ICON(
  //      g_object_ref_sink(gtk_builder_get_object(builder, "tray_icon")));
  self->menu =
      GTK_MENU(g_object_ref_sink(gtk_builder_get_object(builder, "menu")));
  self->popover_menu = GTK_POPOVER_MENU(
      g_object_ref_sink(gtk_builder_get_object(builder, "popover_menu")));
...
static int on_command_line(SuiApplication *self,
  ...
  if (g_variant_dict_lookup(options, G_OPTION_REMAINING, "^as", &urls)) {
    ...
    g_variant_dict_insert(params, "urls", SUI_EVENT_PARAM_STRINGS, urls,
                          g_strv_length(urls));
