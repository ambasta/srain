...
static void preview_error_text(SuiUrlPreviewer *self, const char *text);
static void preview_image(SuiUrlPreviewer *self, GdkPixbuf *pixbuf);

static void on_notify_visible(GObject *object, GParamSpec *pspec,
                              gpointer data);
...
static void sui_url_previewer_init(SuiUrlPreviewer *self) {
  ...
  g_signal_connect(self, "notify::visible", G_CALLBACK(on_notify_visible),
                   NULL);
...
static void sui_url_previewer_class_init(SuiUrlPreviewerClass *class) {
  ...
  obj_properties[PROP_URL] =
      g_param_spec_string("url", "URL", "URL of URL previewer.",
                          NULL, // Default value
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

  obj_properties[PROP_CONTENT_TYPE] = g_param_spec_int(
      "content-type", "Content Type", "The content type pointed to by the URL.",
  ...
  gtk_widget_class_set_template_from_resource(
      widget_class, "/im/srain/Srain/url_previewer.glade");

  gtk_widget_class_bind_template_child(widget_class, SuiUrlPreviewer, expander);
  gtk_widget_class_bind_template_child(widget_class, SuiUrlPreviewer,
                                       expander_label);
...
SuiUrlPreviewer *sui_url_previewer_new(const char *url) {
  return g_object_new(SUI_TYPE_URL_PREVIEWER, "url", url, NULL);
}
...
void sui_url_previewer_preview(SuiUrlPreviewer *self) {
  ...
  if (sui_url_previewer_get_content_type(self) ==
...
static void preview_image(SuiUrlPreviewer *self, GdkPixbuf *pixbuf) {
  ...
  sui_common_scale_size(gdk_pixbuf_get_width(pixbuf),
                        gdk_pixbuf_get_height(pixbuf), THUMBNAIL_SIZE,
...
static void image_event_box_on_button_release(GtkWidget *widget,
  ...
  sui_common_scale_size(gdk_pixbuf_get_width(pixbuf),
                        gdk_pixbuf_get_height(pixbuf), rect.width, rect.height,
                        &width, &height);
  scaled_pixbuf =
      gdk_pixbuf_scale_simple(pixbuf, width, height, GDK_INTERP_BILINEAR);
...
static void session_send_ready(GObject *object, GAsyncResult *result,
  ...
  default:
    buffered_stream =
        G_BUFFERED_INPUT_STREAM(g_buffered_input_stream_new(input_stream));
    g_object_unref(input_stream);
    g_buffered_input_stream_set_buffer_size(buffered_stream, len);
    g_buffered_input_stream_fill_async(buffered_stream, len, G_PRIORITY_LOW,
                                       self->cancel, buffered_stream_fill_ready,
                                       self);
  }
...
static void buffered_stream_fill_ready(GObject *object, GAsyncResult *result,
...

  {
    ...
    if (avail != size) {
      g_buffered_input_stream_fill_async(buffered_stream, avail, G_PRIORITY_LOW,
                                         self->cancel,
                                         buffered_stream_fill_ready, self);
      return;
    }
  }
