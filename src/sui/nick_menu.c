...
void nick_menu_popup(GtkWidget *widget, GdkEventButton *event,
                     const char *nick) {
  int n;
  GList *lst;
  GtkBuilder *builder;
  ...
  whois_menu_item =
      (GtkMenuItem *)gtk_builder_get_object(builder, "whois_menu_item");
  ignore_menu_item =
      (GtkMenuItem *)gtk_builder_get_object(builder, "ignore_menu_item");
  kick_menu_item =
      (GtkMenuItem *)gtk_builder_get_object(builder, "kick_menu_item");
  chat_menu_item =
      (GtkMenuItem *)gtk_builder_get_object(builder, "chat_menu_item");
  invite_menu_item =
      (GtkMenuItem *)gtk_builder_get_object(builder, "invite_menu_item");
  ...
  while (lst) {
    ...
    item = GTK_MENU_ITEM(
        gtk_menu_item_new_with_label(sui_buffer_get_name(lst->data)));
    gtk_widget_show(GTK_WIDGET(item));
    gtk_widget_set_name(GTK_WIDGET(item), "invite_submenu_item");
    g_signal_connect(item, "activate", G_CALLBACK(nick_menu_item_on_activate),
                     (char *)nick);
  ...
  gtk_menu_popup_at_pointer(nick_menu, (const GdkEvent *)event);
