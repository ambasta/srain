...
static void scrolled_window_on_edge_overshot(GtkScrolledWindow *swin,
                                             ...
                                             gpointer user_data);
static void
scrolled_window_vadjustment_on_value_changed(GtkAdjustment *adjustment,
                                             gpointer user_data);
...
static void sui_message_list_init(SuiMessageList *self) {
  ...
  g_signal_connect(gtk_scrolled_window_get_vadjustment(self->scrolled_window),
                   "value-changed",
                   G_CALLBACK(scrolled_window_vadjustment_on_value_changed),
                   self);

  g_object_bind_property(
      gtk_scrolled_window_get_vscrollbar(self->scrolled_window), "opacity",
      self->go_bottom_button, "opacity", 0);
}
...
static void sui_message_list_class_init(SuiMessageListClass *class) {
  ...
  gtk_widget_class_set_template_from_resource(
      widget_class, "/im/srain/Srain/message_list.glade");

  gtk_widget_class_bind_template_child(widget_class, SuiMessageList,
                                       scrolled_window);
...
void sui_message_list_append_message(SuiMessageList *self, SuiMessage *msg,
  ...
  if (self->last_msg && (G_OBJECT_TYPE(msg) == G_OBJECT_TYPE(self->last_msg))) {
...
/* ``scrolled_window_on_edge_overshot()`` and
 * ``scrolled_window_on_edge_reached()`` are used for implement dynamic
 * hide&load messages */
...
static void scrolled_window_vadjustment_on_value_changed(GtkAdjustment *adj,
                                                         gpointer user_data) {
  SuiMessageList *self;
