...
void sui_free_application(SuiApplication *app) { sui_application_exit(app); }
...
SuiBuffer *sui_new_buffer(void *ctx, SuiBufferEvents *events,
                          SuiBufferConfig *cfg) {
  SuiBuffer *buf;
  SrnChat *chat;
...
SuiUser *sui_new_user(void *ctx) { return sui_user_new(ctx); }

void sui_free_user(SuiUser *user) { sui_user_free(user); }

void sui_update_user(SuiBuffer *buf, SuiUser *user) {
  g_return_if_fail(SUI_IS_CHAT_BUFFER(buf));
  g_return_if_fail(user);

  sui_user_list_update_user(sui_chat_buffer_get_user_list(SUI_CHAT_BUFFER(buf)),
                            user);
}
